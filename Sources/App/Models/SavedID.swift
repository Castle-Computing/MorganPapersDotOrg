...
final class MapJSONBuilder {
  ...
  init(entry: docArray){
    ...
    self.longitude = 0
    self.latitude = 0
    let cityname =  self.location.components(separatedBy: ",")[0]
    let index = data_city.index(of: cityname)
    self.date = entry.date ?? ""
    self.datearray = date.components(separatedBy: "-")
    self.description = entry.description ?? ""
    self.title = entry.title ?? ""
    ...
    if index != nil {
      let locationarray: [String : Any] = ["lon": data_longitude[index!], "lat": data_latitude[index!], "zoom": 10, "line": true]
