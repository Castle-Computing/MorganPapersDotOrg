...
public func routes(_ router: Router) throws {
    ...
    router.get("staticjson") { req -> String in
      ...
      var start_date : [String: Any] = ["year": 1900, "month": 1, "day": 1]
      var moretext : [String: Any] = ["headline":"3358254","text":""]
      var media = ["url" : "https://digital.lib.calpoly.edu/islandora/rest/v1/object/rekl:3037/datastream/TN"]
      var events : [String: Any] = ["start_date": start_date, "text": moretext, "media" : media]
      var eventarray = [events, events, events]
      var jsonDic : [String: Any] = ["title": text, "events": eventarray]
    ...
    //
    router.get("dynamicjson") { req -> Future<View> in
      let client = try req.client()
      ...
      var truncated = queryingURL.substring(to: queryingURL.length - 4)
      truncated.append("?rows=15&omitHeader=true&wt=json&start=0")
      ...
      .flatMap { result-> Future<View>  in
          ...
          for entry in result.response.docs{
            ...
            var start_date : [String: Any] = ["year": datearray[0] ?? "", "month": datearray[1] ?? "", "day": datearray[2] ?? ""]
            var titleanddescription : [String: Any] = ["headline":title,"text":description ?? ""]
            var media = ["url" : "https://digital.lib.calpoly.edu/islandora/rest/v1/object/" + entry.pid + "/datastream/TN"]
            var events : [String: Any] = ["start_date": start_date, "text": titleanddescription, "media" : media]
            eventarray.append(events)
          }
          let text = ["headline":"Your Timeline", "text":"A Custom Morgan Letter Timeline"]
          var jsonDic : [String: Any] = ["title": text, "events": eventarray]
          let data = try JSONSerialization.data(withJSONObject: jsonDic, options: .prettyPrinted)
          var jsonString = String(data: data, encoding: String.Encoding.ascii)
          return try req.view().render("json", ResultPage(searchTerm: jsonString ?? "", searchResults: result.response.docs, numResults: result.response.numFound, start: result.response.start, page: 8))
      }
    }
